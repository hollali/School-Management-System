            ...
            <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 overflow-hidden p-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="bg-gray-50/50 rounded-xl p-6">
                        <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Fee Invoice</p>
                        <p class="text-lg font-semibold text-gray-900">{{ $payment->fee->invoice_number ?? '—' }}</p>
                    </div>
                    <div class="bg-gray-50/50 rounded-xl p-6">
                        <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Student</p>
                        <p class="text-lg font-semibold text-gray-900">{{ $payment->fee->student->user->name ?? '—' }}</p>
                    </div>
                    <div class="bg-gray-50/50 rounded-xl p-6">
                        <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Amount</p>
                        <p class="text-lg font-bold text-emerald-600">{{ number_format($payment->amount, 2) }}</p>
                    </div>
                    <div class="bg-gray-50/50 rounded-xl p-6">
                        <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Paid At</p>
                        <p class="text-lg font-semibold text-gray-900">{{ $payment->paid_at ? $payment->paid_at->format('Y-m-d H:i') : '—' }}</p>
                    </div>
                    <div class="bg-gray-50/50 rounded-xl p-6">
                        <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Method</p>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                            @if($payment->method === 'cash') bg-gray-100 text-gray-700
                    ...
                    <div class="bg-gray-50/50 rounded-xl p-6">
                        <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Reference</p>
                        <p class="text-lg font-semibold text-gray-900">{{ $payment->reference ?? '—' }}</p>
                    </div>
                    <div class="bg-gray-50/50 rounded-xl p-6">
                ...
                @if($payment->receipt)
                    <div class="mt-8 border-t border-gray-100 pt-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div class="bg-gray-50/50 rounded-xl p-6">
                                <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Receipt Number</p>
                                <p class="text-lg font-semibold text-gray-900">{{ $payment->receipt->receipt_number ?? '—' }}</p>
                            </div>
                            <div class="bg-gray-50/50 rounded-xl p-6">
                                <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Issued At</p>
                                <p class="text-lg font-semibold text-gray-900">{{ $payment->receipt->issued_at ? $payment->receipt->issued_at->format('Y-m-d H:i') : '—' }}</p>
                            </div>
                            <div class="sm:col-span-2 bg-gray-50/50 rounded-xl p-6">
                                <p class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-1">Notes</p>
                                <p class="text-gray-900">{{ $payment->receipt->notes ?? '—' }}</p>
                            </div>
                        </div>
                        <div class="mt-6">
