<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ __('Edit Feedback') }}</h2>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 p-8">
                <form action="{{ route('assignment-feedback.update', $assignmentFeedback) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <select name="submission_id" id="submission_id"
                                ...
                                @foreach($submissions as $submission)
                                    <option value="{{ $submission->id }}" {{ old('submission_id', $assignmentFeedback->submission_id) == $submission->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="teacher_id" id="teacher_id"
                                ...
                                @foreach($teachers as $teacher)
                                    <option value="{{ $teacher->id }}" {{ old('teacher_id', $assignmentFeedback->teacher_id) == $teacher->id ? 'selected' : '' }}>{{ $teacher->user->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="number" name="score" id="score" value="{{ old('score', $assignmentFeedback->score) }}" step="0.01" min="0" max="100"
                        ...
                        <div>
                            ...
                            <textarea name="comments" id="comments" rows="4"
                                class="block w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 text-sm py-2.5 px-4">{{ old('comments', $assignmentFeedback->comments) }}</textarea>
