<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-bold text-gray-900 dark:text-slate-200">{{ __('New Announcement') }}</h2>
                <p class="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Create a new announcement</p>
            </div>
                ...
                <form action="{{ route('announcements.store') }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        @if(Auth::user()->hasRole('Teacher'))
                            ...
                            <div class="space-y-3">
                                <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 dark:border-slate-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700/50 transition">
                                    <input type="radio" name="target_type" value="all_students" class="text-sky-600 focus:ring-sky-500"
                                        @checked(!old('target_class_id') && !old('target_student_id'))>
                                    <div>
                            ...
                            <div class="mt-4" id="class-select" style="{{ old('target_class_id') && !old('target_student_id') ? '' : 'display: none;' }}">
                                <x-input-label for="target_class_id" :value="__('Select Class')" />
                                <select id="target_class_id" name="target_class_id"
                                    class="mt-1 block w-full border-gray-300 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-sky-500 focus:ring-sky-500 rounded-xl shadow-sm text-sm">
                                    <option value="">Choose a class...</option>
                                    @foreach($classes as $class)
                                        <option value="{{ $class->id }}" @selected(old('target_class_id') == $class->id)>{{ $class->name }}</option>
                                    @endforeach
                                </select>
                                <x-input-error :messages="$errors->get('target_class_id')" class="mt-2" />
                            </div>

                            <div class="mt-4" id="student-select" style="{{ old('target_student_id') ? '' : 'display: none;' }}">
                                ...
                                <select id="target_student_id" name="target_student_id"
                                    ...
                                    @foreach($students as $student)
                                        <option value="{{ $student->id }}" @selected(old('target_student_id') == $student->id)>{{ $student->user?->name }} ({{ $student->admission_number }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select id="target_role" name="target_role"
                                        ...
                                        <option value="admin" @selected(old('target_role') === 'admin')>Administrators Only</option>
                                    </select>
